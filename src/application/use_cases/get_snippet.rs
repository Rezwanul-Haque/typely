use crate::application::dto::SnippetDto;
use crate::domain::SnippetRepository;
use anyhow::Result;
use std::sync::Arc;
use uuid::Uuid;

pub struct GetSnippetUseCase {
    repository: Arc<dyn SnippetRepository>,
}

impl GetSnippetUseCase {
...
mod tests {
    ...
    async fn create_test_use_case() -> (GetSnippetUseCase, CreateSnippetUseCase, TempDir) {
        ...
        let get_use_case = GetSnippetUseCase::new(repository.clone());
