...
pub struct DeleteSnippetUseCase {
    repository: Arc<dyn SnippetRepository>,
}

impl DeleteSnippetUseCase {
    ...
    pub async fn execute(&self, id: Uuid) -> Result<bool> {
        ...
        if let Some(snippet) = snippet {
            ...
            if deleted {
                // Log domain event
                let event = DomainEvent::snippet_deleted(id, snippet.trigger);
...
mod tests {
    ...
    async fn create_test_use_case() -> (DeleteSnippetUseCase, CreateSnippetUseCase, TempDir) {
        ...
        let delete_use_case = DeleteSnippetUseCase::new(repository.clone());
