use crate::application::dto::{CreateSnippetRequest, SnippetDto};
use crate::domain::{Snippet, SnippetRepository, DomainEvent};
use anyhow::Result;
use std::sync::Arc;

pub struct CreateSnippetUseCase {
    repository: Arc<dyn SnippetRepository>,
}

impl CreateSnippetUseCase {
    ...
    pub async fn execute(&self, request: CreateSnippetRequest) -> Result<SnippetDto> {
        ...
        // Log domain event
        let event = DomainEvent::snippet_created(snippet.id, snippet.trigger.clone());
...
mod tests {
    ...
    async fn create_test_use_case() -> (CreateSnippetUseCase, TempDir) {
        ...
        let use_case = CreateSnippetUseCase::new(repository);
        (use_case, temp_dir)
    }
