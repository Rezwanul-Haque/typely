use crate::application::dto::{SnippetListRequest, SnippetListResponse, SnippetDto, PageInfo};
use crate::domain::{SnippetRepository, SnippetQuery, SortBy, SortOrder};
use anyhow::Result;
use std::sync::Arc;

pub struct ListSnippetsUseCase {
    repository: Arc<dyn SnippetRepository>,
}

impl ListSnippetsUseCase {
    ...
    fn convert_request_to_query(&self, request: &SnippetListRequest) -> SnippetQuery {
        ...
        if let Some(ref search_term) = request.search_term {
            query = query.with_search_term(search_term.clone());
        }
        ...
        if let Some(ref sort_order) = request.sort_order {
            match sort_order.as_str() {
                "asc" => query.sort_order = SortOrder::Ascending,
                "desc" => query.sort_order = SortOrder::Descending,
...
mod tests {
    ...
    async fn create_test_use_case() -> (ListSnippetsUseCase, CreateSnippetUseCase, TempDir) {
        ...
        let list_use_case = ListSnippetsUseCase::new(repository.clone());
