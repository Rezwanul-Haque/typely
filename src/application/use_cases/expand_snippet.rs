...
impl ExpandSnippetUseCase {
    ...
    pub async fn execute(&self, request: ExpansionRequest) -> Result<ExpansionResponse> {
        ...
        if expansion_result.success {
            ...
            // Log domain event
            let event = DomainEvent::snippet_expanded(
                updated_snippet.id,
                updated_snippet.trigger.clone(),
                expansion_result.expanded_text.len(),
            );
            ...
            Ok(ExpansionResponse {
                success: false,
                expanded_text: None,
                error_message: expansion_result.error_message,
            })
        }
    }
...
mod tests {
    ...
    async fn create_test_use_case() -> (ExpandSnippetUseCase, CreateSnippetUseCase, TempDir) {
        ...
        let expand_use_case = ExpandSnippetUseCase::new(repository.clone());
    ...
    async fn test_expand_inactive_snippet() {
        ...
        // Deactivate the snippet (this would normally be done through UpdateSnippetUseCase)
