use crate::application::dto::{UpdateSnippetRequest, SnippetDto};
use crate::domain::{SnippetRepository, DomainEvent};
use anyhow::Result;
use std::sync::Arc;

pub struct UpdateSnippetUseCase {
    repository: Arc<dyn SnippetRepository>,
}

impl UpdateSnippetUseCase {
    ...
    pub async fn execute(&self, request: UpdateSnippetRequest) -> Result<SnippetDto> {
        ...
        // Log domain event
        let event = DomainEvent::snippet_updated(snippet.id, snippet.trigger.clone());
...
mod tests {
    ...
    async fn create_test_use_case() -> (UpdateSnippetUseCase, CreateSnippetUseCase, TempDir) {
        ...
        let update_use_case = UpdateSnippetUseCase::new(repository.clone());
