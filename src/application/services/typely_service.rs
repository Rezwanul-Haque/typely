...
pub struct TypelyService {
    pub create_snippet: CreateSnippetUseCase,
    pub update_snippet: UpdateSnippetUseCase,
    pub delete_snippet: DeleteSnippetUseCase,
    pub get_snippet: GetSnippetUseCase,
    pub list_snippets: ListSnippetsUseCase,
    pub expand_snippet: ExpandSnippetUseCase,
    pub import_snippets: ImportSnippetsUseCase,
    pub export_snippets: ExportSnippetsUseCase,
}

impl TypelyService {
    pub async fn new(database_connection: DatabaseConnection) -> Self {
        let repository: Arc<dyn SnippetRepository> = Arc::new(
            SqliteSnippetRepository::new(database_connection.pool().clone())
        );

        Self {
            create_snippet: CreateSnippetUseCase::new(repository.clone()),
            update_snippet: UpdateSnippetUseCase::new(repository.clone()),
            delete_snippet: DeleteSnippetUseCase::new(repository.clone()),
            get_snippet: GetSnippetUseCase::new(repository.clone()),
            list_snippets: ListSnippetsUseCase::new(repository.clone()),
            expand_snippet: ExpandSnippetUseCase::new(repository.clone()),
            import_snippets: ImportSnippetsUseCase::new(repository.clone()),
            export_snippets: ExportSnippetsUseCase::new(repository.clone()),
        }
    }
    ...
    pub async fn import_from_json(&self, json_data: &str, overwrite: bool) -> Result<ImportResult> {
        self.import_snippets.import_from_json(json_data, overwrite).await
    }

    pub async fn export_snippets(&self, request: ExportSnippetsRequest) -> Result<Vec<ImportSnippetData>> {
